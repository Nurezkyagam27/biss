...
Original file is located at
    https://colab.research.google.com/drive/1ppNSb3qug9LYr3jDM_JPyrBXyoaghd7L
"""
...
if page == "🏠 Objek Baru":
    ...
    with col1:
        ...
        if uploaded_file is not None:
            ...
            with col_btn1:
                predict_btn = st.button("🔍 Klasifikasi", use_container_width=True)
            with col_btn2:
            ...
            if model is not None:
                with st.spinner("🔄 Menganalisis gambar..."):
                    ...
                    try:
                        ...
                        gradcam_img = create_gradcam_overlay(image, heatmap)
                    except:
                        gradcam_img = None
                ...
                st.markdown(f"<h2 style='color: #00CED1; text-align: center;'>{predicted_class}</h2>",
                           unsafe_allow_html=True)
                st.markdown(f"<p style='text-align: center; font-size: 1.2rem;'>Confidence: <strong>{confidence:.2f}%</strong></p>",
                           unsafe_allow_html=True)
                ...
                if gradcam_img is not None:
                    ...
                    st.image(gradcam_img, caption='Area fokus model AI', use_column_width=True)
                    st.caption("Warna merah menunjukkan area yang paling diperhatikan model saat membuat prediksi")
                ...
                for idx in top3_idx:
                    ...
                    st.progress(predictions[idx])

                # Chart predictions
                st.markdown("### 📈 Distribusi Probabilitas")
                fig = go.Figure(data=[
                    go.Bar(
                        x=[predictions[i]*100 for i in range(len(class_names))],
                        y=class_names,
                        orientation='h',
                        marker=dict(
                            color=[predictions[i]*100 for i in range(len(class_names))],
                            colorscale='Viridis'
                        )
                    )
                ])
...
# ============================================================================
    ...
    # Dummy data untuk visualisasi (ganti dengan data real dari training)
    ...
    with st.expander("Lihat Detail Model"):
        ...
        **Training:**
        ...
        - Batch size: 32
        - Epochs: Transfer Learning (100) + Fine-tuning (50)
